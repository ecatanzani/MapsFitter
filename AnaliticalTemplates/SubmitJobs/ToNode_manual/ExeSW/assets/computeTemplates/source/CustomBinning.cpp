...
void create_binning(Int_t n_bin_lat,Double_t lat_bin_min,Double_t lat_bin_max,Double_t* &binning,Bool_t cos_scale) {
    ...
    if (cos_scale==false) {
        ...
        Double_t binning_shift = (TMath::Cos(TMath::DegToRad()*lat_bin_max)-TMath::Cos(TMath::DegToRad()*lat_bin_min))/n_bin_lat;
        Double_t prev_bin;
        binning[0]=lat_bin_min;
        for(Int_t idx_b=1; idx_b<=n_bin_lat; idx_b++) {
            prev_bin = TMath::DegToRad()*binning[idx_b-1];
            binning[idx_b] = TMath::RadToDeg()*TMath::ACos(TMath::Cos(prev_bin)+binning_shift);
        }
    }
}
*/
